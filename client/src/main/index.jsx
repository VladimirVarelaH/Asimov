...
function Form(props){
    ...
    function sendHandler(event){
        ...
        if (data.name && data.email && data.date && data.hour && data.hour < 10){
            ...
            if ( (current_month == 12 && ((date[1]== 12 || date[1]==1) && (date[0] == current_year || date[0] == current_year+1))) ||
            (current_month != 12 && ((date[1] == current_month || date[1] == current_month+1) && date[0]==current_year)) ){
                axios.post(
                    'http://localhost:5000/',
                    data
                ).then(res=>{
